...
import signal
import os
import configparser  # Added for saving settings
...
class TransparentWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        print("popover exists:", hasattr(self, "popover"))
    ...
    def on_save_clicked(self, widget=None):
        ...
        opacity_val = int(self.hscale.get_value())
        icon_val = int(self.icons.get_value())
        font_val = int(self.fonts.get_value())
        theme_val = self.themes.get_active_text()

        if not os.path.exists(os.path.dirname(fn.config)):
            os.makedirs(os.path.dirname(fn.config))

        config = configparser.ConfigParser()
        config.read(fn.config)  # Preserve existing sections if any

        if not config.has_section("settings"):
            config.add_section("settings")
        config.set("settings", "opacity", str(opacity_val))
        config.set("settings", "icon_size", str(icon_val))
        config.set("settings", "font_size", str(font_val))

        if not config.has_section("themes"):
            config.add_section("themes")
        config.set("themes", "theme", theme_val)

        try:
            with open(fn.config, "w") as configfile:
                config.write(configfile)
        except Exception as e:
            print(f"Failed to save config: {e}")

        # Update live values
        self.opacity = opacity_val / 100
        self.icon = icon_val
        self.font = font_val
        self.theme = theme_val

        # Hide popover and fully clean up (same as cancel)
        if hasattr(self, 'popover'):
            self.popover.hide()
        self.on_close()  # This removes lockfile + quits GTK properly
...
if __name__ == "__main__":
    ...
    lockfile = "/tmp/i3-logout.lock"
    pidfile = "/tmp/i3-logout.pid"

    # Stale lockfile guard: remove if no running process
    if os.path.isfile(lockfile):
        ...
        if os.path.isfile(pidfile):
            try:
                ...
                os.kill(pid, 0)
                stale = False  # Process exists → real lock
            except (OSError, ValueError, ProcessLookupError):
                pass  # Process dead or bad PID → stale
        if stale:
            print("Detected stale i3-logout lockfile — removing automatically.")
            if os.path.isfile(lockfile):
                os.unlink(lockfile)
            if os.path.isfile(pidfile):
                os.unlink(pidfile)
    ...
    if not os.path.isfile(lockfile):
        ...
        # Final cleanup on normal exit (in case something bypasses on_close)
        if os.path.isfile(lockfile):
            os.unlink(lockfile)
        if os.path.isfile(pidfile):
            os.unlink(pidfile)
    else:
        print("i3-logout is already running.")
        sys.exit(1)